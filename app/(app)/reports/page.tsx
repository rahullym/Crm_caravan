      ...
      <div className="page-container">
        ...
        <div className="dashboard-stats-grid" style={{ marginBottom: 24 }}>
          ...
          <div className="stat-card">
            ...
            <div className="stat-badge green">{wonLeads} deposit paid</div>
          </div>
          <div className="stat-card">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 24 }}>
          ...
          <div className="card">
            <div className="card-header">
              ...
              <span style={{ fontSize: 13, color: "var(--text-muted)" }}>{totalLeads} total</span>
            </div>
            <div style={{ padding: "4px 20px 20px" }}>
              {LEAD_STATUSES.map(statusDef => {
                const row = leadsByStatus.find(r => r.status === statusDef.value)
                const count = row?._count.id ?? 0
                return (
                  <div key={statusDef.value} style={{ marginBottom: 14 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                      <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-main)", display: "flex", alignItems: "center", gap: 8 }}>
                        <span style={{ width: 8, height: 8, borderRadius: "50%", background: statusDef.color, display: "inline-block" }} />
                        {statusDef.label}
                      </span>
                      <span style={{ fontSize: 13, color: "var(--text-muted)" }}>
                        {count} ({totalLeads > 0 ? ((count / totalLeads) * 100).toFixed(0) : 0}%)
                      </span>
                    </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
          ...
          <div className="card">
            <div className="card-header">
              <div className="card-title">Recent Wins (Deposit Paid)</div>
              <Link href="/leads?status=DEPOSIT_PAID" style={{ fontSize: 13, color: "var(--primary)", fontWeight: 600, textDecoration: "none" }}>
                View all →
              </Link>
            </div>
            <div>
